<section xmlns="http://www.w3.org/1999/html">
    ...
    @foreach($seed as $s)
    <form method="post" action="{{ route('seeds.update', $s->id) }}" enctype="multipart/form-data" class="mt-6
    space-y-6">
        @csrf
        @method('put')
        <div>
       ...
       focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                @foreach($categories as $category)
                    <option value="{{$category->id}}" {{$category->id == $s->category_id ?
                    'selected' : '' }}>{{$category->friendly_name}}</option>
                @endforeach
            </select>
